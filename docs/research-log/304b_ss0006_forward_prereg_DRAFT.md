...
> ⛔ **DOC 306 DISPOSITION: NO-GO — do not register Trial 34.** (1) P-3 on the true forward definition: Sharpe-per-bps 0.02407 (−13.8% vs basis A), non-SSR universe drift **−61.9 bps against the short**, mean ticket cost **26.9 bps**; 80% power at ω=0.25 needs a 249 bps (24 mo) / 315 bps (12 mo) tranche-minus-universe gap, above doc 260's contaminated 240 bps upper bound; CEREMONIAL (252) / UNDISCRIMINATING (504) at the default prior; every number re-derived by an independent skeptic. (2) **The frozen model id `Qwen/Qwen3-235B-A22B-Instruct-2507-tput` is no longer served** — absent from Together's /v1/models (271 listed, 2026-09-22) and answering 400 'Unable to access non-serverless model' since 2026-07-13 — so under §3.7 the instrument is retired before freeze; any substitute model is a new pre-registration with its own power case. The EX-99 rule (§3.2) and the §2.4 time-zone note below are corrected for the record, so a successor draft inherits them. See doc 306.
>
...
- **The text the model reads is selected differently from doc 260's.** §3.2 picks the exhibit by EDGAR document Type, taking the lowest-numbered EX-99. Doc 260 picked it by file name (`ex.?99`).
  - **Why the change is necessary.** On P-5 the name rule would have given the model the 8-K cover page instead of the press release for 23 of 64 eligible events (35.9%). It would have done the same for 138 of 400 seeded 2025 Item-2.02 filings (34.5%), all of which carried an EX-99 exhibit. A cover page repeats the Item 2.02 boilerplate and contains no results. Scoring a third of events on it tests an input the hypothesis is not about. The error could not be fixed later either, because §3.4 forbids re-scoring.
  - **Why the change is acceptable.**
    - The rule reads only the filing index. It runs before any score exists, cannot see outcomes, and is pinned by F-12 and unit tests.
    - On the filings where the name rule did find an exhibit, the two rules chose the same document 328 times out of 329 (P-5 72/72; 2025 256/257). The one disagreement is a case where the name rule picked the earnings-summary tables (EX-99.2) instead of the release (EX-99.1).
    - The forward test already does not inherit doc 260's magnitude (§7.2).
  - **A caution about doc 260 itself.** If doc 260's filings behaved like these samples, roughly a third of its scores were made on cover pages. That was not measured, because doc 260's texts and scores were not opened.
...
- ⚠ **Corrected doc 306 (P-3 skeptic): not always.** Some submissions-JSON `acceptanceDateTime` "Z" values are really Eastern time: 10 of 10 checked had an EDGAR index time 4–5 h later than the converted time; 238 of 16,826 2025 events convert to before 06:00 ET (EDGAR closed). About 0.2% of eligible events get a D_e one session EARLY, which contradicts the robustness bullet below (a misparse can move D_e earlier, not only later). No look-ahead results, because T_e still falls after the true acceptance. A successor must take A from the EDGAR index page (or cross-check it) rather than trust the JSON suffix.
...
**3.2 Text** (doc 260's strip and cap; the exhibit is selected by EDGAR type, a declared deviation, §1.4):
- From the filing's `{accession}-index.htm` (the "Document Format Files" table only; "Data Files" rows are ignored), take the rows whose EDGAR **Type** matches `^EX-99(\.\d+)?$` (case-insensitive) and whose document name ends in .htm, .html or .txt. Choose the **lowest exhibit number**: bare EX-99 counts as 0, so EX-99 < EX-99.1 < EX-99.2, and the suffix is compared as an integer (EX-99.9 < EX-99.10). Ties go to the lower Seq. If no row qualifies, take `primaryDocument` and record **PRIMARY-FALLBACK**. A PRIMARY-FALLBACK is counted but is not a hole, because the Item 2.02 text is then in the body of the 8-K. The reference implementation is `select_text_document` (doc 306, `ex99_rule.py`), pinned by F-12.
  - **Declared deviation from doc 260** (P:136-138), which matched file *names* in `index.json` against `ex.?99` and took the first hit. File names carry no reliable exhibit token (`exhibit991.htm`, `a20260731-brazeincxq227ear.htm`, `q22025pressrelease.htm`), so the name rule took the 8-K cover instead of the exhibit in 23 of 64 eligible P-5 events (35.9%) and in 143 of 400 seeded 2025 Item-2.02 filings (35.8%). In 138 of those 143 filings an EX-99 existed. The type rule falls back 0 times in 64 and 5 times in 400, and each of those 5 filings has no EX-99 of any kind (doc 306).
  - If the index page cannot be fetched or has no Document Format Files table, the runner retries until the §3.6 deadline. It never takes `primaryDocument` because a fetch failed. An event that has no score by the deadline is LATE-SCORE.
...
| **F-12** | §3.2 text | Index-page rows (Seq, document, Type). **A:** (1, primary.htm, 8-K, with the iXBRL marker), (2, ex99-2slides.htm, EX-99.2), (3, a2027q4ear.htm, EX-99.1), (4, exhibit99.pdf, EX-99), plus a Data Files row (5, ex99-schema.htm, EX-99). **B:** (1, p.htm, 8-K), (2, x10.htm, EX-99.10), (3, x9.htm, EX-99.9). **C:** (1, p.htm, 8-K), (2, exhibit991.pdf, EX-99.1), (3, logo.jpg, GRAPHIC). Also: HTML with script and style blocks and entities; a 12,000-character body; a 250-character body | A → a2027q4ear.htm; B → x9.htm; C → p.htm, with PRIMARY-FALLBACK counted; the pinned stripped string; 9,000 characters kept; TEXT-SHORT | doc-260 name regex on the sorted names → A ex99-2slides.htm; fallback not flagged → C chosen but PRIMARY-FALLBACK count 0; Seq order → A ex99-2slides.htm; extension ignored → A exhibit99.pdf, C exhibit991.pdf; suffix compared as a string → B x10.htm; Data Files table parsed → A ex99-schema.htm; no truncation; the short text scored |
...
| P-3 | power on the true forward definition | Sharpe per bps recomputed on 2025 Item-2.02 events with real `acceptanceDateTime`, 15:30 entries, the frozen one-per-issuer-per-session rule (no 14-day de-dup), ETB thinning, the Rule 201 exclusion and moving-block CIs; §7.1 is frozen on that result; no LLM, no tranche outcome | **done (doc 306) → NO-GO.** 16,826 Item-2.02 8-Ks → 16,771 after one-per-issuer-per-session → **11,769 eligible** (3,198 tickers; ~47/session; Jun/Sep/Dec thin at ~1.3–1.5 tranche tickets/session). **Rule 201 at T_e on 1-minute bars: 24.54%** (the draft's 10.6/22.1% were not upper bounds). Non-SSR universe mean gross **−61.9 bps** (moving-block 90% [−114, −6]); hedged sd 10.19%. **Sharpe/bps 0.02407** iid, 0.02331 block-10. **Mean cost 26.92 bps** (median 18.96; 648 measured SIP tickets). X80 net at 504: 135 (ω=1) / 160 (ω=0.25); gross gap needed 224 / 249 bps. Skeptic re-derived all five load-bearing numbers |
| P-4 | model access | the HTTP 402 is diagnosed (key, account or endpoint); Together is restored with the frozen id (Pierce's spend); MODEL_ECHO is recorded; canaries are scored 3× and recorded | **blocked; re-diagnosed (doc 306)**: ~~HTTP 403 on every endpoint~~. The 403 was Cloudflare rejecting urllib's default User-Agent. With a normal UA, `GET /v1/models` returns 200 and 1-token chats on both the frozen id and Llama-3.3-70B return **HTTP 402 `credit_limit`**. The key is valid and the account is out of credit. The five canaries were never written in this draft; doc 306 writes them. MODEL_ECHO and the canary run wait on credit |
| P-5 | plumbing dry run, 10 sessions | EDGAR counts; borrow snapshots at T_e − 5 min, including an early close if one falls in the run; NBBO `feed=sip` present at 15:30 for ≥ 98% of names; the account-default feed used by `true_nbbo_cost.py` identified, with floors re-measured on SIP if it differs; **counts only**: the SSR rate over all events and the primary-document fallbacks. No dry-run score may seed a window | **retrospective parts done (doc 305); forward part NOT run.** Over 2026-09-09..09-22 (off-season), EDGAR EFTS matches the daily index on all 11 published days (0 differences) and the submissions JSON on 2,343/2,343 items. There were 106 Item-2.02 events and **64 eligible**. The **default feed is SIP** (identical data), and the doc-303 floors reproduce on SIP. **SIP ±60 s at 15:30: 63/64 = 98.4% on the eligible denominator** (all 101 mapped events: 91.1%; §2.8 mark: 97.0%), so the eligible-only reading of 'names' must be frozen. **SSR in force at T_e: 20/64 = 31.2%** (Wilson 21.2–43.4%), above the 10.56% planning rate; the §2.3 flag must use 1-minute bars, because daily lows miss post-close prints. **The frozen EX-99 regex fell back to the cover document on 23/64 (35.9%)**, though all 64 carry an EX-99. **Median SIP quoted spread at 15:30 on eligible events: 20.32 bps**, against the 10.5 bps in the §5.4 illustration, so re-check P-3/§7.1. The forward spec (borrow snapshots at T_e−5 min, discovery lag, watchdog) is written, not scheduled |
| P-6 | Alpaca borrow fee | measured from Alpaca's schedule or a live statement; freeze max(measured, 1.2127%) | **done (doc 305).** Measured from Alpaca's published fee schedule (revised 2026-09-17; PDF sha256 7bc75e3c…d71b12): **easy-to-borrow borrow fee 0.00%** ("Free (no locate or stock borrow fees on Easy-to-Borrow securities)"). **Frozen b = max(0.00%, 1.2127%) = 1.2127% = 4.81 bps per 10-session ticket.** Paper does not simulate borrow fees, and no API exposes a rate. ETB share of the eligible universe today: 92.4% of events |
...
- ~~a fix to the `ex.?99` regex, which is kept frozen from doc 260 and counted in P-5.~~ Adopted in doc 306 as a type-based selection (§3.2, §1.4).