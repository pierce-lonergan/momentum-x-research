...
| P-4 | model access | the HTTP 402 is diagnosed (key, account or endpoint); Together is restored with the frozen id (Pierce's spend); MODEL_ECHO is recorded; canaries are scored 3× and recorded | **blocked; diagnosed (doc 305)**: the Together key now returns **HTTP 403 on every endpoint, including GET /v1/models**. That is key- or account-level, not endpoint-specific (the August 402s were on the -tput model only). Restoring access is Pierce's. MODEL_ECHO and the canaries are not run |
